...
import styles from 'styles/types/imageBlock.module.scss';
    ...
    const BackgroundImg = ({ images, size, isPreview }: {images: string[], size: {w: number, h: number}, isPreview: boolean}) => {
        ...
        if(isPreview) {
            ...
            if (window.devicePixelRatio > 1) {
                <img className={styles.wideImg} alt={src} src={src} />
            }
            // desktop
            return <div className={styles.backgroundImg} style={{backgroundImage: `url(${src})`, backgroundPosition: 'top center', backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundAttachment: 'fixed', width: '100vw', height: '100vh' }} ></div>
        }
    }
